...
import com.iotekclass.buscompoents.service.filter.FilterService;
import com.iotekclass.buscompoents.service.permission.ManageAccountLogService;
...
public class CustomManageShiroSession {
	...
	public void generateUserSessions(ManageAccount manageAccount) {
		...
		if (manageRoles != null && manageRoles.size() > 0) {
			...
			//把用户权限资源放在memcache中
			if (memManager.exists(manageAccount.getId()+UserConstants.ACCOUNT_PERMISSION_KEY)) {
				memManager.replace(manageAccount.getId()+UserConstants.ACCOUNT_PERMISSION_KEY,
						urls);
			} else {
				memManager.add(manageAccount.getId()+UserConstants.ACCOUNT_PERMISSION_KEY,
						urls);
			}
		}
		...
		// 登录成功之后记录userId jsessionid。添加manage后缀，防止和其它项目冲突

		if (memManager.exists(String.valueOf(manageAccount.getId()+"_operation-consumer"))) {
			memManager.replace(String.valueOf(manageAccount.getId())+"_operation-consumer", jSessionId);
		} else {
			memManager.add(String.valueOf(manageAccount.getId())+"_operation-consumer", jSessionId);
		}
		if (logger.isDebugEnabled()) {
			logger.debug("userId：" + userId + "  jsessionid：" + memManager.get(String.valueOf(manageAccount.getId())+"_operation-consumer"));
		}
